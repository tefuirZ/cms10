    ...
    public function data()
    {
        ...
        $where = [];
         if(!empty($param['vod_year'])){
            $where['vod_year'] = ['eq',$param['vod_year']];
        }
        if(!empty($param['type'])){
        ...
        if(!empty($param['repeat'])){
            if($param['page'] ==1){
                ...
                Db::query('CREATE TABLE IF NOT EXISTS `'.config('database.prefix').'tmpvod` as (SELECT min(vod_id) as id1,vod_name as name1,concat(vod_name,type_id,vod_year,vod_area) as name_type,type_id as tid1,vod_year as year1,vod_area as area1 FROM '.config('database.prefix').'vod GROUP BY name_type HAVING COUNT(name_type)>1)');
            }
            $order='vod_name asc';
		...
		$vod_years = Db::query("select DISTINCT vod_year from mac_vod where vod_year <> '' and CONVERT(vod_year,SIGNED)>2010 and CONVERT(vod_year,SIGNED)<=".date("Y")." order by CONVERT(vod_year,SIGNED) desc limit 99999");


        $this->assign('vod_years',$vod_years);
