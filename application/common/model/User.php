    ...
    public function reward($where, $fee_points=0)
    {
        ...
        if($fee_points>0 && $GLOBALS['config']['user']['reward_status'] == '1'){
            ...
            if(!empty($GLOBALS['config']['user']['reward_ratio']) && !empty($user['user_pid'])){            // 如果奖励开启 并且用户存在上级
                $points = floor($fee_points / 100 * $GLOBALS['config']['user']['reward_ratio']); // 奖励积分数
                    ...
                    if ($r1){
                        ...
                        if (self::isAgents($r1)){
                            ...
                            if($r){
                                ...
                                $data['glog_remarks'] = '用户【'.$user['user_id'].'、'.$user['user_name'].'】充值'.$fee_points.'积分，获得奖励'.$points.'金币';
                            ...
                            if($r){
                                ...
                                $data['plog_remarks'] = '用户【'.$user['user_id'].'、'.$user['user_name'].'】充值'.$fee_points.'积分，获得奖励'.$points.'积分';
                                model('Plog')->saveData($data);
                            }
                        }
                    }
