...
class Ulog extends Base {
        ...
        foreach($list as $k=>&$v){
            ...
            if($v['ulog_mid']==1){
                ...
                $v['data'] = [
                    ...
                    'pic_thumb'=>$vod_info['info']['vod_pic_thumb'],
    ...
    public function saveData($data)
    {
        ...
        if(!empty($data['ulog_id'])){
            ...
            if ($data['ulog_mid'] == 1 && $data['ulog_type'] == 2){
                $re = model('User')->where('user_id', $data['user_id'])->setInc('vod_store_num', 1);
            }
        }
        if(false === $res){
    ...
    public function delData($where)
    {
        ...
        if ($re){
            $r = model('User')->where('user_id', $re['user_id'])->setDec('vod_store_num', 1);
        }
