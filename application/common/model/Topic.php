...
class Topic extends Base {
    ...
    public function listCacheData($lp)
    {
        ...
        if($by=='rnd'){
            $data_count = mac_data_count(0,'all','topic');
            $data_min = mac_data_count(0,'min','topic');
            $maxi = $data_count-$lp['num'];
            if($maxi<0){ $maxi=0; }
            $randi = @mt_rand($data_min, $data_min + $maxi);
            if($randi<0) { $randi=0; }
            $where['topic_id'] = ['gt',$randi];
            $by='time';
            $order='asc';
        }
    ...
    public function infoData($where,$field='*',$cache=0)
    {
        ...
        if($GLOBALS['config']['app']['cache_core']==0 || $cache==0 || empty($info['topic_id']) ) {
            ...
            if (!empty($info['topic_rel_vod'])) {
                ...
                $order = 'vod_time desc';
                $field = '*';
                $res = model('Vod')->listData($where, $order, 1, 60, 0, $field);
                if ($res['code'] == 1) {
            ...
            if (!empty($info['topic_tag'])) {
                ...
                $order = 'art_time desc';
                $field = '*';
                $res = model('Art')->listData($where, $order, 1, 60, 0, $field);
                if ($res['code'] == 1) {
