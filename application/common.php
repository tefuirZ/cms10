...
{
    $arr = [1=>'浏览',2=>'收藏',3=>'想看',4=>'点播',5=>'下载',6=>'评分'];
    return $arr[$data];
}

function mac_get_plog_type_text($data)
{
    $arr = [1=>'积分充值',2=>'注册推广',3=>'访问推广',4=>'三级分销',7=>'积分升级',8=>'积分消费',9=>'积分提现',10=>'签到奖励','11'=>'评论奖励','12'=>'评分奖励','13'=>'弹幕奖励'];
    return $arr[$data];
}


function mac_get_glog_type_text($data)
{
    $arr = [1=>'金币充值',2=>'金币消费',3=>'三级分销',4=>'金币提现',5 => '原路退回账户'];
    return $arr[$data];
}

function mac_get_glog_withdraw_apply_ttype_text($data){
    $arr = [1=>'支付宝',2=>'微信'];
    return $arr[$data];
}

function mac_get_os_text($data)
{
    $arr = [1=>'安卓',2=>'IOS'];
    return $arr[$data];
}
...
function mac_path_to_imageurl($path){
    $preg = "/^http(s)?:\\/\\/.+/";
    ...
    {
        $ret= 'http://'.mac_url_img($_SERVER[HTTP_HOST].'/'.$path);
    }
    return $ret;
}
    ...
    switch ($model)
    {
        ...
        case 'vod/play':
            ...
            if($config['view']['vod_play'] >=2){
              ...
              //$url = url($model,['id'=>$info[$id],'nid'=>$param['nid']]);
...
function getUserVideoTimes($userId){
    if($userId == 0){
        return 0;
    }
    $data=[];
    ...
    $used_times = model('Vlog')::where('user_id',$userId)
        ->field('vod_id,nid,source')
        ->where('last_view_time', '>=', strtotime(date('Y-m-d') . '00:00:00'))
        ->where('last_view_time', '<=', strtotime(date('Y-m-d') . '23:59:59'))
        ->group('vod_id,nid,source')
        ->select();
    
    $data['leave_times'] = $data['user_view_count']-count($used_times);
