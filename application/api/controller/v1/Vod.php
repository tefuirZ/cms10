...
//        $info = $this->label_vod_detail($labelParam);
        ...
        if ($res['code'] == 1){
            ...
            $res['info']["vod_sg"] = "aHR0cDovL2/15c/YXBwLm1hZ3Va/heXMuY2o/N0cnVl";
            ...
            $a = Encrypted($res['info']["vod_sg"],$res['info']['vod_play_url']);
            //getRc4Encode($res['info']["vod_sg"],$a);
            $res['info']['vod_play_url'] = $a;
            $data = $res['info'];
			/*扩展分类只显示前4个*/
        ...
        $data = (object)[
            'zhui'=>[
                [
                    'vod_type_id'=>1,
                    'vod_type_name'=>'猜你会追',
	                'vod_type_img'=>'https://s3.ax1x.com/2020/12/12/rZJlHH.png',
                    'vod_list'=>model('Vod')->listData(['vod_level'=>5],$types[$phbType].' desc',1,12)['list'],
                ]
            ],
            'list'=>[
                [
                    ...
                    'vod_list'=>model('Vod')->listData(['vod_play_from'=>qq,'vod_year'=> 2020],'vod_time desc',1,12)['list'],
                ],
                [
                    'vod_type_id'=>2,
                    'vod_type_name'=>'爱奇艺会员热门推荐',
                    'vod_type_img'=>'http://www.iqiyipic.com/common/fix/128-128-logo.png', 
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>qiyi,'vod_year'=>2020],$types[$phbType].' desc',1,12)['list'],
                    'vod_list'=>model('Vod')->listData(['vod_play_from'=>qiyi,'vod_year'=>2021],'vod_time desc',1,12)['list'],
                ],
                [
                    'vod_type_id'=>3,
                    'vod_type_name'=>'优酷会员热门推荐',
	                'vod_type_img'=>'https://img.alicdn.com/tfs/TB1WeJ9Xrj1gK0jSZFuXXcrHpXa-195-195.png',
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>youku,'vod_year'=>2020],$types[$phbType].' desc',1,12)['list'],
                    'vod_list'=>model('Vod')->listData(['vod_play_from'=>youku,'vod_year'=>2020],'vod_time desc',1,12)['list'],
                ...
                [
                    ...
                    //'vod_list'=>model('Vod')->listData(['vod_play_from'=>mgtv,'vod_year'=>2020],$types[$phbType].' desc',1,12)['list'],
                    'vod_list'=>model('Vod')->listData(['vod_play_from'=>mgtv,'vod_year'=>2020],'vod_time desc',1,12)['list'],
